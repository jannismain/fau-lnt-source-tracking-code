...
if nargin > 3 && exist('movement','var') && ~(ischar(movement))
    src_movement = movement;
else
    src_movement = [0 0 0];
end

%% handling optional argument 'samples'
if nargin > 3 && exist('samples','var') && ~(ischar(samples))
    src_samples = samples;
else
...
if not(nargin < 3)
    ...
    if size(x,2)==2
        ...
        src_x_trajectory = linspace(x(1),x(2),src_samples).';
    elseif src_movement(1) ~= 0
        fprintf("      -> x is VECTOR ([%d %d], moves by %d)\n", x, x+src_movement(1), src_movement(1));
        src_x_trajectory = linspace(x,x+src_movement(1),src_samples).';
    elseif size(x,2)==1
        fprintf("      -> x is SCALAR (%d)\n", x);
        src_x_trajectory = ones(src_samples,1)*x;
    else
    ...
    if size(y,2)==2
        ...
        src_y_trajectory = linspace(y(1),y(2),src_samples).';
    elseif src_movement(2) ~= 0
        fprintf("      -> y is VECTOR ([%d %d], moves by %d)\n", y, y+src_movement(2), src_movement(2));
        src_y_trajectory = linspace(y,y+src_movement(2),src_samples).';
    elseif size(y,2)==1
        fprintf("      -> y is SCALAR (%d)\n", y);
        src_y_trajectory = ones(src_samples,1)*y;
    else
    ...
    if size(z,2)==2
        ...
        src_z_trajectory = linspace(z(1),z(2),src_samples).';
    elseif src_movement(3) ~=0
        fprintf("      -> z is VECTOR ([%d %d], moves by %d)\n", z, z+src_movement(3), src_movement(3));
        src_z_trajectory = linspace(z,z+src_movement(3),src_samples).';
    elseif size(z,2)==1
        fprintf("      -> z is SCALAR (%d)\n", z);
        src_z_trajectory = ones(src_samples,1)*z;
    else
        error("Argument 'z' has to be either 1- or 2-dimensional!\n")
    end
else
    error("This function requires 3 input arguments x, y, z.")
end
