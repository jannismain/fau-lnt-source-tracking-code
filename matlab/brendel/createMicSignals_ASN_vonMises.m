...
%--------------------------------------------------------------
% Illustration of simulated environment and the corresponding
% synthetic IRs
%--------------------------------------------------------------
...
for idx_pair = 1:cfg.n_pairs
    for p=1:size(cfg.synth_room.sloc, 1)
        h(:, p, :,idx_pair) = rir_generator(cfg.c, cfg.fs, cfg.synth_room.mloc(:,:,idx_pair), cfg.synth_room.sloc(p, :), cfg.synth_room.dim, cfg.synth_room.t60, cfg.synth_room.Nh, 'omnidirectional', cfg.synth_room.order, 3)';
    end
end
