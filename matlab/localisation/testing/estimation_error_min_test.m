...
loc_est_assorted = [3.9 1.7;
                    3.0 4.0];
tic;
[~,loc_est,est_err] = evalc('estimation_error_min(S, loc_est_assorted);');
...
loc_est_assorted = [3.9 4.0;
                    ...
                    1.1 3.9];
tic;
[~,~,est_err] = evalc('estimation_error_min(S, loc_est_assorted);');
assert(est_err(1)==0)
assert(est_err(2)==0)
...
for n=1:10
    ...
    [~,~,est_err] = evalc('estimation_error_min(S, S_est_perm);');
...
for m=1:50
    for n=2:5
        ...
        for i=1:size(all_perm_S, 1)
            [~,~, loc_err(i,:)] = evalc('estimation_error_min(S,S_est(all_perm_S(i,:),:));');
        end
...
for m=1:50
    for n=2:5
        ...
        loc_err = zeros(size(all_perm_S,1), n);
        for i=1:size(all_perm_S, 1)
            [~,~, loc_err(i,:)] = evalc('estimation_error_min(S(all_perm_S(i,:),:),S_est);');
        end
        mean_err = mean(loc_err, 2);
        assert(round(min(mean_err),2)==round(max(mean_err),2));
    end
...
S_est = [1.5,2.9;
         ...
         2.2,4.9;
         1.1,3.9];
[~,loc_est,est_err] = evalc('estimation_error_min(S, S_est);');
