...
pd.set_option('display.precision', 6, 'display.width', 300,
              'display.chop_threshold', 0.0001, 'display.expand_frame_repr',
              False, 'display.max_rows', None)
...
EVALUATIONS = [
    'base', 'em-iterations', 'min-distance', 'reflect-order', 'T60', 'noise',
    'wd', 'var-fixed'
]
PARAMETERS = {
    's': None,
...
DEFAULT_LINE_PLOT_ARGS = {
    ...
    6,
    'color': [
        lms_red, "orange", "black", "darkgrey", "blue", "magenta", "green",
        "yellow", "lightgray"
    ]
}
...
def _get_col_name(s, post):
    return sum([("x{}{},y{}{}".format(n, post, n, post)).split(',')
...
def print_summary(df):
    ...
    summary = df.groupby('n-sources').agg(DICT_SUMMARY).rename(
        columns={
            'x1': 'n',
            'em': 'em-iterations'
        })
    print(summary.transpose())
    for col in [x for x in PARAMETERS.keys()
                if x != "s"]:  # Print additional notes for these columns
        warn = False
        if df[col].min() != df[col].max():
            col_values = df.pivot_table(
                "x1", index=[col], columns=["n-sources"], aggfunc="count")
            if col_values.min().min() == col_values.max().max():
                n = col_values.min().min()
            else:
                if (col_values.min(axis=1).values -
                        col_values.max(axis=1)).sum() != 0:
                    warn = True
                n = list(col_values.min(axis=1).values)
            print("NOTE: Data contains range of {col} ({values}, n={n})"
                  .format(
                      col=col,
                      values=[str(x) for x in list(col_values.index.values)],
                      n=n))
            if warn:
                print(
                    "WARN: Unbalanced n_sources for '{}'. There may be a trial running at the moment!".
                    format(col))
                print(col_values)
...
def calculate_helpers(df: pd.DataFrame):
    df_helpers = (
        df.loc[:, "x1":"x7":2] - df.loc[:, "x1est":"x7est":2].values
    ).rename(
...
def matlab2pandas(dirname=EVALUATIONS,
                  filename=NAME_DATA_FILES,
                  save_to=None,
                  summary=True):
    dfs = []
    ...
    for dirname in dirnames:
        ...
        for f in files:
            ...
            # prepare DataFrame
            df = pd.DataFrame(
                list(csv.reader(open(f, 'r'), delimiter='\t')), dtype=float)
            df.drop(
                df.columns[[n_sources * 4 + n_sources]], axis=1,
                inplace=True)  # drops empty column
        ...
        df.rename(
            columns={'s': 'n-sources',
                     'refl-ord': 'reflect-order'},
            inplace=True)
        df['SNR'] = df['SNR'].apply(int)
        df['em'] = df['em'].apply(int)
        df['n-sources'] = df['n-sources'].apply(int)
        df["T60"].apply(round, 2)
        df["err-mean"] = df.loc[:, "err1":_get_err_col_name(n_sources)[
            -1]].mean(axis=1)
        df["err-total"] = df.loc[:, "err1":_get_err_col_name(n_sources)[
            -1]].sum(axis=1)
...
for desc in EVALUATIONS:
    dfs.append(
        matlab2pandas(
            dirname=desc, save_to=path.join(PATH_ROOT, desc), summary=True))
df = pd.concat(dfs)
